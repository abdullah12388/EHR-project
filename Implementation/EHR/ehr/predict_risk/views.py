...
def PredictRisk(request,pk):
    ...
    #     patient_id = request.session['p_id']
    patient_id = pk
    if request.method == 'POST':
        form = Predict_Form(data=request.POST)
        profile = get_object_or_404(patient, id=pk)
    ...
    if predicted:
        return render(request, 'predict.html',
                      {'form': form,'predicted': predicted,'user_id':patient_id,'predictions':predictions,'result':result,'colors':colors})
    else:
        form = Predict_Form()
        return render(request, 'predict.html',
                      {'form': form,'predicted': predicted,'user_id':patient_id,'predictions':predictions})
